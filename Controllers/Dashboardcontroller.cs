...
public class DashboardController : ControllerBase
{
    ...
    public IActionResult ObtenerDashboard()
    {
        ...
        kpiCmd.CommandText = """
            SELECT
              ...
              COUNT(*) FILTER (WHERE plazo_p2 IS NOT NULL
                               AND   plazo_p2::date < CURRENT_DATE)                         AS vencidos_p2,
        ...
        // ── 4. Últimos 10 PMs realizados (P1 y P2 mezclados) ──
        ...
        mesCmd.CommandText = """
            ...
            SELECT id, id_equipo, nombre_dispositivo, ubicacion, planta,
                   plazo_p2::text AS plazo_fecha, categoria_color, 2 AS periodo
            FROM public.mantenimientos_preventivos
            WHERE plazo_p2 IS NOT NULL
              AND plazo_p2::date >= date_trunc('month', CURRENT_DATE)
              AND plazo_p2::date <  date_trunc('month', CURRENT_DATE) + interval '1 month'
        ...
        vencidosCmd.CommandText = """
            ...
            SELECT id, id_equipo, nombre_dispositivo, ubicacion, planta,
                   plazo_p2::text AS plazo_fecha, fecha_realizacion_p2 AS ultimo_pm, categoria_color, 2 AS periodo
            FROM public.mantenimientos_preventivos
            WHERE plazo_p2 IS NOT NULL AND plazo_p2::date < CURRENT_DATE
            LIMIT 25
        ...
        // ── 8. PM por mes — últimos 12 meses (P1 + P2 desglosado) ──
        ...
        // ── 10. Salud del Parque ───────────────────────────
        // Un equipo se evalúa por el PEOR plazo que tenga (P1 o P2 activo).
        // • Al corriente  : todos sus plazos activos son > fin de este mes
        // • Por vencer    : al menos un plazo cae dentro del mes actual (y ninguno vencido)
        // • Vencido       : al menos un plazo ya pasó de hoy
        // Equipos sin ningún plazo registrado no se cuentan en el semáforo.
        ...
        saludCmd.CommandText = """
            WITH plazos AS (
                SELECT
                    ...
                    LEAST(
                        ...
                        CASE WHEN plazo_p2 IS NOT NULL THEN plazo_p2::date ELSE NULL END
                    ) AS peor_plazo
                FROM public.mantenimientos_preventivos
            ...
            SELECT
                ...
                COUNT(*) FILTER (WHERE peor_plazo >= date_trunc('month', CURRENT_DATE)
                                 AND   peor_plazo <  date_trunc('month', CURRENT_DATE) + interval '1 month'
                                 AND   peor_plazo >= CURRENT_DATE)                                AS por_vencer_mes,
        ...
        // Índice de salud 0-100: penaliza vencidos fuerte, por vencer suave
        double indice = 100.0;
        if (totalConPlazo > 0)
        {
