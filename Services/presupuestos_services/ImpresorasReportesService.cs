...
{private readonly DbConnectionPool _pool;
    ...
    {await using var conn = await _pool.OpenAsync();
        await using var cmd = new NpgsqlCommand("""
            ...
            CREATE TABLE IF NOT EXISTS impresoras_info (
                id               SERIAL PRIMARY KEY}, {impresora        TEXT}, {modelo           TEXT}, {numero_de_serie  TEXT}, {ip               TEXT}, {ubicacion        TEXT}, {planta           TEXT}, {identificador    TEXT}, {numero           INTEGER
            );
    ...
    {await using var conn = await _pool.OpenAsync();
        ...
        {lista.Add(new
            {
                ID                   = reader.GetInt32(0)}, {FOLIO                = Str(reader}, {1)}, {FECHA                = Str(reader}, {2)}, {PLANTA               = Str(reader}, {3)}, {IMPRESORA            = Str(reader}, {4)}, {AREA                 = Str(reader}, {5)}, {REPORTE              = Str(reader}, {6)}, {QUIEN_REPORTA        = Str(reader}, {7)}, {ESTATUS              = Str(reader}, {8)}, {FECHA_DE_REALIZACION = Str(reader}, {9)}, {COMENTARIOS          = Str(reader}, {10)});
        }
    ...
    {await using var conn = await _pool.OpenAsync();
        ...
        while (await r.ReadAsync())
        {
            lista.Add(new
            {
                ID                = r.GetInt32(0)}, {USUARIO           = r.GetString(1)}, {FECHA             = r.GetDateTime(2).ToString("yyyy-MM-dd HH:mm:ss")}, {REGISTRO_ANTERIOR = r.IsDBNull(3) ? null : r.GetString(3)}, {REGISTRO_NUEVO    = r.IsDBNull(4) ? null : r.GetString(4)});
        }
        return lista;
    }
    ...
    {await using var conn = await _pool.OpenAsync();
        ...
        {lista.Add(new
            {
                ID              = reader.GetInt32(0)}, {IMPRESORA       = Str(reader}, {1)}, {MODELO          = Str(reader}, {2)}, {NUMERO_DE_SERIE = Str(reader}, {3)}, {IP              = Str(reader}, {4)}, {UBICACION       = Str(reader}, {5)}, {PLANTA          = Str(reader}, {6)}, {IDENTIFICADOR   = Str(reader}, {7)}, {NUMERO          = reader.IsDBNull(8) ? (int?)null : reader.GetInt32(8)});
        }
    ...
    {ExcelPackage.License.SetNonCommercialPersonal("IT Control Center");
        ...
        string[] headers =
        [
            "ID"}, {"FOLIO"}, {"FECHA"}, {"PLANTA"}, {"IMPRESORA"}, {"ÁREA"}, {"REPORTE"}, {"QUIEN REPORTA"}, {"ESTATUS"}, {"FECHA DE REALIZACIÓN"}, {"COMENTARIOS"
        ];
    ...
    {ExcelPackage.License.SetNonCommercialPersonal("IT Control Center");
        ...
        string[] headers =
        [
            "ID"}, {"IMPRESORA"}, {"MODELO"}, {"NO. SERIE"}, {"IP"}, {"UBICACIÓN"}, {"PLANTA"}, {"IDENTIFICADOR"}, {"NÚMERO"
        ];
