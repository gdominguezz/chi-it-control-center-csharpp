...
public class RadiosNFService
{
    ...
    [
        ...
        "REQUISITOR","DISPONIBLE","FECHA_SALIDA","ASIGNADO_A",
    ...
    private async Task InicializarTablaAsync()
    {
        ...
        await using var cmd = new NpgsqlCommand("""
            ...
            CREATE TABLE IF NOT EXISTS radios_nf_historial (
                id                SERIAL PRIMARY KEY,
                radio_id          INTEGER NOT NULL REFERENCES radios_nf(id) ON DELETE CASCADE,
        ...
        await using var cmd = new NpgsqlCommand("""
            ...
            VALUES
                (@id_unico, @oc, @folio, @fecha_registro::date, @recibido_por,
                 @subcategoria, @marca, @modelo, @no_serie, @cantidad,
                 @observaciones, @proveedor, @costo, @moneda, @vida_util,
                 @requisitor, @disponible, @fecha_salida::date, @asignado_a,
                 @destino_planta, @personal_it_asigna)
            RETURNING id
            """, conn);
    ...
    private static byte[] GenerarExcel(List<string?[]> rows)
    {
        ...
        [
            ...
            "CANTIDAD","OBSERVACIONES","PROVEEDOR","COSTO","MONEDA",
            "VIDA ÚTIL","REQUISITOR","DISPONIBLE","FECHA SALIDA",
            "ASIGNADO A","DESTINO PLANTA","PERSONAL IT ASIGNA"
        ];
